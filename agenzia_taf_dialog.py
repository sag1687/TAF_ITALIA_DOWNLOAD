...
class AgenziaTafDialog(QDialog, FORM_CLASS):
    ...
    def _update_ui_lang(self):
        ...
        self.tabWidget.setTabText(
            0, _t(L, "Download e Mappa", "Download and Map")
        )
        self.tabWidget.setTabText(
            1, _t(L, "Informazioni e Licenza", "Information and License")
        )
        self.tabWidget.setTabText(
            2, _t(L, "Fonti Originali TAF", "Original TAF Sources")
        )
        idx = self.tabWidget.indexOf(self.tab_origini)
        if idx >= 0:
            self.tabWidget.setTabText(
                idx, _t(L, "Origini Locali", "Local Origins")
            )
        ...
        self.groupBox_search.setTitle(
            _t(L, "Ricerca Comune", "Municipality Search")
        )
        ...
        self.label_disclaimer_outlier.setText(
            _t(L, _OUTLIER_HTML_IT, _OUTLIER_HTML_EN)
        )
        ...
        self.btn_apri_plugin.setText(
            _t(L, "Apri Repository GitHub", "Open GitHub Repository")
        )
        self.lbl_altri_plugin.setText(
            _t(
                L,
    ...
    def setup_fonti_tab(self):
        ...
        for regione, province in regioni:
            ...
            for sigla, nome in province:
                ...
                html += (
                    f"<li><b>{sigla}</b> - {nome}: " f'{", ".join(links)}</li>'
                )
            html += "</ul>"
    ...
    def _retranslate_origini(self):
        if not hasattr(self, "origini_info_label"):
            return
        L = self.lang
        self.origini_info_label.setText(
            _t(
                L,
        ...
        self.btn_export_modello.setText(
            _t(L, "📥 Scarica Modello Vuoto", "📥 Download Empty Template")
        )
        self.btn_import_file.setText(
            _t(L, "📤 Importa da File", "📤 Import from File")
        )
        self.btn_add_orig.setText(
            _t(L, "➕ Aggiungi Origine", "➕ Add Origin")
        )
        self.btn_rem_orig.setText(
            _t(L, "🗑️ Rimuovi Selezionata", "🗑️ Remove Selected")
        )
        self.btn_save_orig.setText(
            _t(L, "💾 Salva Configurazione", "💾 Save Configuration")
        )
    ...
    def add_origine_row(
        self, codice="", nota="", lat=0.0, lon=0.0, fe=0, fn=0
    ):
            ...
            try:
                piccole[codice] = {
                    "nota": (
                        ...
                        if self.table_origini.item(row, 1)
                        else ""
                    ),
                    "lat": float(_cell(2)),
                    "lon": float(_cell(3)),
    ...
    def export_origini_template(self):
        filepath, _f = QFileDialog.getSaveFileName(
            self,
            _t(self.lang, "Salva Modello Origini", "Save Origins Template"),
    ...
    def import_origini_file(self):
        filepath, _f = QFileDialog.getOpenFileName(
            self,
            _t(self.lang, "Seleziona File Origini", "Select Origins File"),
            os.path.expanduser("~"),
        ...
        try:
            ...
            if importati > 0:
                QMessageBox.information(
                    self,
                    _t(
                        self.lang, "Importazione completata", "Import complete"
                    ),
                    _t(
                        self.lang,
